...
<div class="h-screen w-screen flex items-center justify-center">
	...
	<button class="btn variant-filled mr-2 focus:outline-none" on:click={handleShareClick}
		>Share</button
	>
	<button class="btn variant-filled-primary" on:click={handleClick}>clicks: {clicks}</button>
	<button
		class="btn variant-filled-secondary ml-2 bg-transparent outline-none border-none"
		on:click={handleSetClick}>SET</button
	>
</div>
